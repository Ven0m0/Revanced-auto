...
- **Temp/Cache:** Use deterministic temp paths under `$TEMP_DIR` (see helpers in `scripts/lib/network.sh`) with `trap ... RETURN`/`EXIT`. For caches, use `cache_path=$(get_cache_path "key")` then `cache_is_valid "$cache_path" <ttl>`.
