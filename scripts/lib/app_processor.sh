...
_app_execute_build() {
  ...
  if [[ "${__aeb_args[arch]}" == both ]]; then
    ...
    (
      BUILD_LOG_FILE="build/${table_name}-arm64-v8a.md"; export BUILD_LOG_FILE
      build_rv "$args_file"
    ) &
    local pid=$!
    JOB_PIDS+=("$pid")
    ...
    (
      BUILD_LOG_FILE="build/${table_name}-arm-v7a.md"; export BUILD_LOG_FILE
      build_rv "$args_file"
    ) &
    local pid=$!
    JOB_PIDS+=("$pid")
    ...
    (
      BUILD_LOG_FILE="build/${table_name}.md"; export BUILD_LOG_FILE
      build_rv "$args_file"
    ) &
    local pid=$!
    JOB_PIDS+=("$pid")
